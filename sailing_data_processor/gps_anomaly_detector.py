...
def process_gps_data(df: pd.DataFrame, detection_methods: Optional[List[str]] = None, 
                   correction_method: str = 'linear') -> pd.DataFrame:
    """
    GPSデータの異常値検出と修正を行うワンステップ関数（互換レイヤー）
    ...
    # 警告を表示
    warnings.warn(
        "process_gps_data関数は後方互換性のために提供されています。"
        "新しいアプリケーションでは sailing_data_processor.anomaly パッケージを使用してください。",
        DeprecationWarning,
        stacklevel=2
    )
    ...
    if detection_methods is None:
        detection_methods = ['z_score', 'speed', 'acceleration', 'distance']
    
    # GPSAnomalyDetectorクラスを使用
    detector = create_anomaly_detector('gps')
    
    # 検出
    result_df = detector.detect_anomalies(df, methods=detection_methods)
    ...
    # 修正
    return detector.fix_anomalies(result_df, method=correction_method)
