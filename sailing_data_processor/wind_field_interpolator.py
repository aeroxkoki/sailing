...
class WindFieldInterpolator:
    ...
    def add_wind_field(self, time_point_or_field: Union[datetime, Dict[str, Any]], wind_field: Optional[Dict[str, Any]] = None):
        """
        風の場データを追加
        
        Parameters:
        -----------
        time_point_or_field : datetime or Dict[str, Any]
            時間点またはwind_fieldオブジェクト（time属性を含む）
        wind_field : Dict[str, Any], optional
            風の場データ (time_point_or_fieldがdatetimeの場合に必要)
        """
        if isinstance(time_point_or_field, datetime):
            # 従来の使用法: add_wind_field(time_point, wind_field)
            time_point = time_point_or_field
            if wind_field is None:
                raise ValueError("wind_field parameter is required when time_point is specified")
            self.wind_field_data[time_point] = wind_field.copy()
        else:
            # 新しい使用法: add_wind_field(wind_field_with_time)
            wind_field = time_point_or_field
            if 'time' not in wind_field:
                raise ValueError("wind_field must contain 'time' attribute")
            time_point = wind_field['time']
            self.wind_field_data[time_point] = wind_field.copy()
    ...
    def _gp_interpolate(self, target_time: datetime, resolution: int) -> Dict[str, Any]:
        ...
        try:
            ...
            epsilon = 1e-5  # 小さなノイズ
            X_noisy = X.copy()
            X_noisy += np.random.normal(0, epsilon, X_noisy.shape)
            ...
            # モデルの学習
            gp_sin.fit(X_noisy, y_sin)
            gp_cos.fit(X_noisy, y_cos)
            gp_speed.fit(X_noisy, y_speed)
    ...
    def _rbf_interpolate(self, target_time: datetime, resolution: int) -> Dict[str, Any]:
        ...
        try:
            # RBF補間器の作成 - 精度問題（Qhullエラー）対策のため小さなノイズを追加
            # 各座標に小さなランダムノイズを追加して座標の完全一致を防ぐ
            epsilon = 1e-5  # 小さなノイズ
            noisy_data = data_points.copy()
            noisy_data += np.random.normal(0, epsilon, noisy_data.shape)
            
            rbf_sin = Rbf(noisy_data[:, 0], noisy_data[:, 1], noisy_data[:, 2], 
                        wind_dir_sin, function='multiquadric')
            rbf_cos = Rbf(noisy_data[:, 0], noisy_data[:, 1], noisy_data[:, 2], 
                         wind_dir_cos, function='multiquadric')
            rbf_speed = Rbf(noisy_data[:, 0], noisy_data[:, 1], noisy_data[:, 2], 
    ...
    def _idw_interpolate(self, target_time: datetime, resolution: int) -> Dict[str, Any]:
        ...
        if len(time_diffs) == 1:
            ...
            return self._resample_wind_field(nearest_field, resolution, qhull_options)
        ...
        # 両方のフィールドをリサンプリング
        field1_resampled = self._resample_wind_field(field1, resolution, qhull_options)
    ...
    def _resample_wind_field(self, wind_field: Dict[str, Any], resolution: int, qhull_options: str = None) -> Dict[str, Any]:
        """
        風の場をリサンプリング
