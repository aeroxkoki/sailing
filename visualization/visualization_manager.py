...
class VisualizationManager:
    ...
    def __init__(self):
        ...
        # レイアウト設定
        self._layout_config = {
            "layout_type": "default",  # "default", "map_focused", "chart_focused", "split", "dashboard", "playback"
    ...
    def register_component(self, component_id: str, component: Any, component_type: str) -> None:
        ...
        component : Any
            コンポーネントオブジェクト
        component_type : str
            コンポーネントのタイプ（"map", "chart", "timeline", "playback", "control"）
        """
    ...
    def get_components_by_type(self, component_type: str) -> Dict[str, Dict]:
        ...
        Dict[str, Dict]
            指定タイプのコンポーネント情報辞書
        """
        return {comp_id: comp_info for comp_id, comp_info in self._components.items() 
                if comp_info["type"] == component_type}
    ...
    def render_layout(self, show_titles: bool = True, custom_heights: Optional[Dict[str, int]] = None) -> None:
        """
        ...
        show_titles : bool, optional
            コンポーネントタイトルを表示するかどうか, by default True
        custom_heights : Optional[Dict[str, int]], optional
            コンポーネントごとの高さ指定（ピクセル）, by default None
        """
        ...
        # コンポーネントの高さ設定
        heights = custom_heights or {}
        
        # レンダリング関数（共通処理を抽出）
        def render_component(comp_id, comp_info, container):
            with container:
                if show_titles:
                    title = comp_info.get("component").name if hasattr(comp_info.get("component"), "name") else comp_id
                    st.subheader(title)
                
                # カスタム高さがあれば適用
                height = heights.get(comp_id)
                if height and hasattr(comp_info.get("component"), "render") and callable(getattr(comp_info.get("component"), "render")):
                    # 高さパラメータをサポートしているか確認
                    import inspect
                    render_sig = inspect.signature(comp_info.get("component").render)
                    if "height" in render_sig.parameters:
                        comp_info.get("component").render(height=height)
                    else:
        ...
        if layout_type == "default":
            ...
            for comp_id, comp_info in components_to_render.items():
                render_component(comp_id, comp_info, st.container())
            ...
            if other_components:
                ...
                for i, (comp_id, comp_info) in enumerate(other_components.items()):
                    render_component(comp_id, comp_info, cols[i])
            ...
            for comp_id, comp_info in left_components.items():
                render_component(comp_id, comp_info, col1)
            
            # 右側のコンポーネント
            for comp_id, comp_info in right_components.items():
                render_component(comp_id, comp_info, col2)
        
        elif layout_type == "dashboard":
            # ダッシュボードレイアウト: 柔軟な配置
            # コンポーネントのタイプに基づいてグループ化
            map_components = {k: v for k, v in components_to_render.items() if v["type"] == "map"}
            chart_components = {k: v for k, v in components_to_render.items() if v["type"] == "chart"}
            control_components = {k: v for k, v in components_to_render.items() if v["type"] == "control"}
            timeline_components = {k: v for k, v in components_to_render.items() if v["type"] == "timeline"}
            playback_components = {k: v for k, v in components_to_render.items() if v["type"] == "playback"}
            other_components = {k: v for k, v in components_to_render.items() 
                              if v["type"] not in ["map", "chart", "control", "timeline", "playback"]}
            
            # プレイバックコントロールは一番上に表示
            if playback_components:
                for comp_id, comp_info in playback_components.items():
                    render_component(comp_id, comp_info, st.container())
            ...
            # チャートとタイムラインを横に並べる
            if chart_components or timeline_components:
                chart_col_count = min(len(chart_components), 2)  # 最大2カラム
                
                if chart_col_count > 0:
                    st.markdown("### データビュー")
                    chart_cols = st.columns(chart_col_count)
                    
                    # チャートを表示
                    for i, (comp_id, comp_info) in enumerate(chart_components.items()):
                        col_idx = i % chart_col_count
                        render_component(comp_id, comp_info, chart_cols[col_idx])
                
                # タイムラインを表示
                if timeline_components:
                    st.markdown("### タイムラインビュー")
                    for comp_id, comp_info in timeline_components.items():
                        render_component(comp_id, comp_info, st.container())
            
            # コントロールは下部に表示
            if control_components:
                st.markdown("### コントロールパネル")
                control_cols = st.columns(min(len(control_components), 3))
                for i, (comp_id, comp_info) in enumerate(control_components.items()):
                    col_idx = i % len(control_cols)
                    render_component(comp_id, comp_info, control_cols[col_idx])
            
            # その他のコンポーネント
            if other_components:
                st.markdown("### その他のビュー")
                other_col_count = min(len(other_components), 2)
                other_cols = st.columns(other_col_count)
                for i, (comp_id, comp_info) in enumerate(other_components.items()):
                    col_idx = i % other_col_count
                    render_component(comp_id, comp_info, other_cols[col_idx])
        
        elif layout_type == "playback":
            # プレイバック特化レイアウト
            playback_components = {k: v for k, v in components_to_render.items() if v["type"] == "playback"}
            map_components = {k: v for k, v in components_to_render.items() if v["type"] == "map"}
            chart_components = {k: v for k, v in components_to_render.items() if v["type"] == "chart"}
            timeline_components = {k: v for k, v in components_to_render.items() if v["type"] == "timeline"}
            other_components = {k: v for k, v in components_to_render.items() 
                              if v["type"] not in ["playback", "map", "chart", "timeline"]}
            ...
            # マップとチャートを横に並べる
            if map_components or chart_components:
                # マップとチャートの分割比率
                map_ratio = 3
                chart_ratio = 2
                
                if map_components and not chart_components:
                    # マップのみの場合
                    for comp_id, comp_info in map_components.items():
                        render_component(comp_id, comp_info, st.container())
                elif chart_components and not map_components:
                    # チャートのみの場合
                    for comp_id, comp_info in chart_components.items():
                        render_component(comp_id, comp_info, st.container())
                else:
                    # マップとチャートを分割表示
                    col1, col2 = st.columns([map_ratio, chart_ratio])
            ...
            # 横並びレイアウト: すべてのコンポーネントを横に並べる
            cols = st.columns(len(components_to_render))
            for i, (comp_id, comp_info) in enumerate(components_to_render.items()):
                render_component(comp_id, comp_info, cols[i])
